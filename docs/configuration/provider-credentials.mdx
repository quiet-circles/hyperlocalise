---
title: "Provider credentials"
description: "Set credentials for OpenAI, Anthropic, Gemini, LM Studio, Ollama, Groq, and remote TMS adapters."
---

## LLM providers

### OpenAI

```bash
export OPENAI_API_KEY="your-openai-api-key"
```

### Gemini

```bash
export GEMINI_BASE_URL="https://generativelanguage.googleapis.com/v1beta/openai"
export GEMINI_API_KEY="your-gemini-api-key"
```

### Anthropic

```bash
export ANTHROPIC_BASE_URL="https://api.anthropic.com/v1"
export ANTHROPIC_API_KEY="your-anthropic-api-key"
```

### LM Studio

```bash
export LM_STUDIO_BASE_URL="http://127.0.0.1:1234/v1"
export LM_STUDIO_API_KEY="lm-studio"
```

### Ollama

```bash
export OLLAMA_BASE_URL="http://127.0.0.1:11434/v1"
export OLLAMA_API_KEY="ollama"
```

### Groq

```bash
export GROQ_BASE_URL="https://api.groq.com/openai/v1"
export GROQ_API_KEY="your-groq-api-key"
```

## Storage adapters

### POEditor

```bash
export POEDITOR_API_TOKEN="your-poeditor-token"
```

### Lokalise

```bash
export LOKALISE_API_TOKEN="your-lokalise-token"
```

### Crowdin

```bash
export CROWDIN_API_TOKEN="your-crowdin-token"
```

### Smartling

```bash
export SMARTLING_USER_SECRET="your-smartling-user-secret"
```

Smartling also requires `storage.config.userIdentifier` in `i18n.jsonc`.
If you use Smartling file mode (`storage.config.mode="files"`), the same credential is used for file import/export workflows.

## Security guidance

- Prefer environment variables over hardcoded tokens.
- Use `.env` tooling only if it is excluded from version control.
- Rotate tokens regularly and scope them to least privilege.
