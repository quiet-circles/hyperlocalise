...
When a catalog declares `sourceLanguage`, each parsed/cloned entry must include that source locale in `localizations`; otherwise `run` fails with a parse/marshal error.
