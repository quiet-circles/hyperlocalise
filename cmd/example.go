package cmd

import (
	"fmt"

	"github.com/quiet-circles/hyperlocalise/internal/convert"
	"github.com/quiet-circles/hyperlocalise/pkg/example"
	"github.com/spf13/cobra"
)

const (
	numberOfArgs = 2
)

type exampleOptions struct {
	multiply bool
	add      bool
}

func defaultExampleOptions() *exampleOptions {
	return &exampleOptions{}
}

func newExampleCmd() *cobra.Command {
	o := defaultExampleOptions()

	cmd := &cobra.Command{
		Use:          "example",
		Short:        "example subcommand which adds or multiplies two given integers",
		SilenceUsage: true,
		Args:         cobra.ExactArgs(numberOfArgs),
		RunE:         o.run,
	}

	cmd.Flags().BoolVarP(&o.multiply, "multiply", "m", o.multiply, "multiply")
	cmd.Flags().BoolVarP(&o.add, "add", "a", o.add, "add")

	return cmd
}

func (o *exampleOptions) run(cmd *cobra.Command, args []string) error {
	if err := o.validateOperations(); err != nil {
		return err
	}

	values, err := o.parseArgs(args)
	if err != nil {
		return err
	}

	if o.multiply {
		if _, writeErr := fmt.Fprintf(cmd.OutOrStdout(), "%d\n", example.Multiply(values[0], values[1])); writeErr != nil {
			return fmt.Errorf("write multiply output: %w", writeErr)
		}
	}

	if o.add {
		if _, writeErr := fmt.Fprintf(cmd.OutOrStdout(), "%d\n", example.Add(values[0], values[1])); writeErr != nil {
			return fmt.Errorf("write add output: %w", writeErr)
		}
	}

	return nil
}

func (o *exampleOptions) validateOperations() error {
	if o.add == o.multiply {
		return fmt.Errorf("exactly one operation flag must be set: --add or --multiply")
	}

	return nil
}

func (o *exampleOptions) parseArgs(args []string) ([]int, error) {
	values := make([]int, 2) //nolint:mnd

	for i, a := range args {
		v, err := convert.ToInteger(a)
		if err != nil {
			return nil, fmt.Errorf("error converting to integer: %w", err)
		}

		values[i] = v
	}

	return values, nil
}
